...
type PeripheralBus struct {
	...
	inputOwners  [8]string
	outputOwners [24]string
}
...
func (b *PeripheralBus) Attach(binding PeripheralBinding) error {
	...
	for _, output := range binding.Outputs {
		index := output.Port - 8
		b.io.outputCallbacks[index] = output.Handler
		b.outputOwners[index] = binding.Name
	}
	return nil
}
...
func (b *PeripheralBus) Bindings() []PortBinding {
	...
	for index, owner := range b.outputOwners {
		if owner != "" {
			bindings = append(bindings, PortBinding{Peripheral: owner, Direction: IODirectionOutput, Port: byte(index + 8)})
		}
	}
...
func (b *PeripheralBus) outputOwner(port byte) string {
	index := port - 8
	if b.outputOwners[index] != "" {
		return b.outputOwners[index]
	}
	if b.io.outputCallbacks[index] != nil {
		return "callback-esterno"
	}
	return ""
}
