...
type TraceEvent struct {
	...
	Before       cpu.CPU8008           `json:"before"`
	After        cpu.CPU8008           `json:"after"`
...
type Debugger struct {
	...
	inputBreakpoints  [8]bool
	outputBreakpoints [24]bool
...
func (d *Debugger) AddOutputBreakpoint(port byte) error {
	...
	d.outputBreakpoints[port-8] = true
	return nil
}
...
func (d *Debugger) hitIOBreakpoint(accesses []IOAccess) bool {
	for _, access := range accesses {
		...
		if access.Direction == IODirectionOutput && d.outputBreakpoints[access.Port-8] {
			return true
		}
	}
	return false
