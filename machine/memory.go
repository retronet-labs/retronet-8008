...
// MemoryBus applica una mappa ROM/RAM allo spazio indirizzi a 14 bit.
...
func validateMemoryRegion(region MemoryRegion) error {
	if region.Start > cpu.AddressMask || region.End > cpu.AddressMask {
		return fmt.Errorf("regione memoria %q fuori dallo spazio 14 bit", region.Name)
	}
...
// Read legge un byte, mascherando l'indirizzo come il bus fisico dell'8008.
